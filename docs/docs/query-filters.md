...
nodes = [
  ...
  { id: 2, post: {title: 'How to debug Gatsby', author: { name: 'Clarissa' } } }
...
Gatsby supports a subset of MongoDB comparators, namely:

- `eq`: Every node where the result value is exactly the filter value.
- `ne`: Every node where the result value is anything except for the filter value.
- `in`: Every node where the result value matches any element in given array
- `nin`: Every node where the result value matches none of the elements in given array
- `lt`: Every node where the result value is lower than the filter value
- `lte`: Every node where the result value is lower than or equal to the filter value
- `gt`: Every node where the result value is greater than the filter value
- `gte`: Every node where the result value is greater than or equal to the filter value
- `regex`: Every node where the result value matches given JavaScript regex
- `glob`: Every node where the result value matches given [micromatch](https://github.com/micromatch/micromatch) pattern
...
- `lt`, `lte`, `gt`, `gte`
  ...
  - Can only be used with numbers, strings, booleans, and null
  - `lt` and `gt` will never match anything for `null`
...
The actual performance of filtering is a combination of the number of nodes you have to search through, the kind of comparator being used, and how many unique filter paths a query consists of.
...
Before applying filters, all fields of any node that a filter wants to match should be completely resolved. As such, before applying a filter all nodes will go through a final resolve step which populates the `node.__gatsby_resolved` field. This means your custom resolver may be invoked multiple times even if fewer (or zero) nodes are actually returned by a filter step.
